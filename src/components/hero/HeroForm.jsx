...
const HeroForm = ({ openContact }) => {
  ...
  const handleSubmit = () => {
    ...
    if (!phone) newErrors.phone = "Mobile number required";
    else if (!phoneRegex.test(phone))
      newErrors.phone = "Enter valid number (+919876543210)";

    if (!selected) newErrors.selected = "Please select insurance type";
    ...
    <QuoteCard>
      <InputRow>
        ...
        <div style={{ flex: 1 }}>
          ...
          {errors.selected && (
            <p style={{ color: "red", fontSize: "12px" }}>{errors.selected}</p>
          )}
        </div>
      </InputRow>

      <QuoteButton onClick={handleSubmit}>GET A FREE QUOTE</QuoteButton>
